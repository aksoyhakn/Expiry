package com.afgdevlab.expirydate.utils.transition
...
class ActivityTransition(builder: Builder) {
    ...
    fun start(delay: Int) {
        mActivity?.let { it.hideKeyboard() }
        Handler(Looper.getMainLooper()).postDelayed({ startAct(intent) }, delay.toLong())
    }
    ...
    fun startWithResult(code: Int) {
        mActivity?.let { it.hideKeyboard() }
        startActWithResult(intent, code)
    }

    private fun startActWithResult(intent: Intent?, code: Int) {
        if (mClass == null) {
            throw IllegalStateException("Target class can not be null!")
        }
        if (mActivity != null && intent != null) {
            val sdk = Build.VERSION.SDK_INT
            if (sdk >= Build.VERSION_CODES.JELLY_BEAN && mActivityOptionsCompat != null) {
                mActivity.startActivityForResult(intent, code, mActivityOptionsCompat.toBundle())
            } else {
    ...
    class Builder(mActivity: Activity, mClass: Class<*>?) {
        ...
        internal var mTransition: Transition = Transition.TransitionSlide()
